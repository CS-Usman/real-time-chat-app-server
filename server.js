...
io.on("connection", async (socket) => {
    ...
    if (Boolean(userId)) {
        await User.findByIdAndUpdate(userId, { socketId: socketId, status: "Online" });
    }
    ...
    socket.on("start_conversation", async (data) => {
        ...
        if (existingConversation.length === 0) {
            ...
            newChat = await OneToOneMessage.findById(newChat._id).populate("participants", "firstName lastName _id email status");
    ...
    socket.on("get_messages", async (data, callback) => {
        const { messages } = await OneToOneMessage.findById(data.conversationId).select("messages");
        callback(messages);
    })
    ...
    socket.on("text_message", async (data) => {
        ...
        const newMessage = {
            to, from, type, text: message, createdAt: Date.now()
        }
